...
│
├── panels/<platform>/<module>/ KiCad faceplate projects, grouped by platform
│                               (hagiwo-mod1/, hagiwo-mod2/, rabid-audio-clk/;
│                               + blanks/ and house-style plates)
...
- `firmwares/<platform>/<name>/<name>.ino` — one sketch per module, grouped into
  platform folders (`hagiwo-mod1/`, `hagiwo-mod2/`, `hagiwo-30/`); a few ports
  (`rabid-audio-*`) still sit directly under `firmwares/`. The root `Makefile`
  discovers any same-named `<name>/<name>.ino` at either depth; the folder
  basename is the flat build target (its `fwdir` helper maps it back to the real
  nested path), and the name prefix picks the board (`mod2-` → RP2350, everything
  else → AVR Nano, which is also what the ATmega328P `rabid-audio-*` ports build
  against).
...
1. Naming is kebab-case and mirrored across layers: firmware
   `firmwares/hagiwo-mod2/mod2-vco/mod2-vco.ino`, Rack
   `rack-plugins/src/mod2-vco.cpp`, panel `panels/hagiwo-mod2/mod2-vco/`, core
   `SynthCore/src/VcoCore.h` (PascalCase `<Name>Core.h`), model `modelVCO`. Only
   the firmware and panel layers carry the platform folder; the Rack `.cpp` and
   the core header stay flat.
...
3. Every Rack module `.cpp` opens with a block comment documenting the
   ...
   (see `firmwares/hagiwo-mod2/mod2-vco/mod2-vco.ino`). Keep this.
