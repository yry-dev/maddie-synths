#include "plugin.hpp"
#include <SquareVcoCore.h>  // Shared square-wave VCO core (also used by mod2-square_vco firmware)

/*
	SquareVCO — square-wave VCO with sine LFO vibrato and chiptune mode.

	Port of firmwares/mod2-square_vco/mod2-square_vco.ino (HAGIWO Mod2, RP2350).
...
struct SquareVCOWidget : ModuleWidget {
	SquareVCOWidget(SquareVCO* module) {
		setModule(module);
		setPanel(createPanel(asset::plugin(pluginInstance, "res/SquareVCO.svg")));
		...
		const float cx = 15.24f;  // 6 HP centre

		addChild(createLightCentered<MediumLight<GreenLight>>(mm2px(Vec(cx, 18.5f)), module, SquareVCO::CHIPTUNE_LIGHT));

		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(cx, 30.f)),  module, SquareVCO::TUNE_PARAM));
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(cx, 47.f)),  module, SquareVCO::OCTAVE_PARAM));
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(cx, 64.f)),  module, SquareVCO::VIB_DEPTH_PARAM));

		addParam(createParamCentered<VCVButton>(mm2px(Vec(cx, 82.f)), module, SquareVCO::CHIPTUNE_PARAM));

		// Jacks: row 1 (IN1, CV), row 2 (IN2, OUT) — matches Claves layout
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(10.16f, 98.f)),   module, SquareVCO::IN1_INPUT));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(20.32f, 98.f)),   module, SquareVCO::CV_INPUT));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(10.16f, 113.f)),  module, SquareVCO::IN2_INPUT));
		addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(20.32f, 113.f)), module, SquareVCO::AUDIO_OUTPUT));
	}
};

Model* modelSquareVCO = createModel<SquareVCO, SquareVCOWidget>("SquareVCO");
